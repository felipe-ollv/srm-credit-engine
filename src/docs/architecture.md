...
Este documento apresenta a arquitetura da entrega Sênior. No estado atual, frontend, autenticação, pricing, cedentes, recebíveis e currency engine estão implementados; liquidação em lote, reporting e seus artefatos operacionais permanecem planejados. Os diagramas usam a notação C4 nos níveis de contexto e contêiner e detalham, em seguida, os módulos e as fronteiras hexagonais do backend.
...
- O PostgreSQL já assegura constraints de cedentes, recebíveis e snapshots cambiais; idempotência, locking otimista e registros imutáveis de liquidação pertencem à próxima fatia transacional.
